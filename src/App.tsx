...
import BlocklyWorkspace, { type MetricasQuestao } from './components/BlocklyWorkspace';
...
export default function App() {
  ...
  const [metricasGlobais, setMetricasGlobais] = useState({ 
    ...
    testesTotais: 0,
    trocasTempoTotais: 0,
    blocosTotais: 0, // NOVO: Total de blocos usados
    granular: [] as any[] 
  });
  ...
  const avancarQuestao = (m: MetricasQuestao) => {
    setMetricasGlobais(prev => ({
      errosTotais: prev.errosTotais + m.erros,
      ...
      granular: [...prev.granular, { 
        id: questoesSessao[indiceAtual].id, 
        err: m.erros, 
        t: m.tempo, 
        test: m.testes, 
        sw_t: m.trocasTempo,
        blk: m.blocos // Adiciona os blocos à matriz granular
      }]
    }));
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-800 p-4">
        <form onSubmit={handleAdminLogin} className="w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl">
          ...
          <button type="submit" className="w-full mb-3 cursor-pointer rounded-xl bg-indigo-600 py-4 font-bold text-white transition-transform hover:bg-indigo-700 active:scale-95">Autenticar</button>
          <button type="button" onClick={() => setTelaAtual('home')} className="w-full py-3 font-bold text-slate-500 hover:text-slate-800">Voltar para o Início</button>
        </form>
      </div>
    );
  }
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50 p-4">
        <form onSubmit={handleLoginPilotos} className="w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl">
          ...
          <button type="submit" className="w-full mb-3 cursor-pointer rounded-xl bg-blue-600 py-4 font-bold text-white transition-transform hover:scale-105 active:scale-95">Entrar no Laboratório 🚀</button>
          <button type="button" onClick={() => setTelaAtual('home')} className="w-full py-3 font-bold text-slate-500 hover:text-slate-800">Voltar</button>
        </form>
      </div>
    );
  }
    ...
    return (
      <div className="h-screen w-screen bg-slate-50 p-4">
        <main className="mx-auto flex h-full w-full max-w-6xl flex-col overflow-hidden rounded-xl border-4 border-slate-200 bg-white shadow-lg">
          <BlocklyWorkspace questao={questoesSessao[indiceAtual]} onVoltar={() => setTelaAtual('lobby')} onProxima={avancarQuestao} progresso={`${indiceAtual + 1}/${questoesSessao.length}`} baloes={indiceAtual} />
        </main>
      </div>
    );
  }
  ...
  if (telaAtual === 'podio') {
    const dadosQR = JSON.stringify({ 
      ...
      err: metricasGlobais.errosTotais, 
      ...
      <div className="flex h-screen items-center justify-center bg-slate-800 p-4">
        <div className="flex w-full max-w-4xl overflow-hidden rounded-3xl bg-white shadow-2xl">
          ...
          <div className="p-10 w-1/2 flex flex-col justify-center">
            ...
            <p className="text-lg text-slate-600 mb-8"><span className="font-bold text-indigo-600">{pilotos.p1}</span> e <span className="font-bold text-indigo-600">{pilotos.p2}</span> finalizaram!</p>
            
            <h4 className="font-bold text-slate-400 mb-3 uppercase text-xs tracking-wider">Métricas da Sessão</h4>
            <div className="space-y-3 mb-8 text-sm">
              ...
              <div className="flex justify-between border-b pb-2"><span className="text-slate-500">Trocas por Tempo Estourado:</span><span className="font-bold text-orange-500">{metricasGlobais.trocasTempoTotais}</span></div>
              <div className="flex justify-between border-b pb-2"><span className="text-slate-500">Tempo Total de Resolução:</span><span className="font-bold text-emerald-600">{formatarTempo(metricasGlobais.tempoTotal)}</span></div>
            </div>
            <button onClick={() => { setPilotos({ p1: '', p2: '' }); setTelaAtual('home'); }} className="w-full rounded-xl bg-slate-800 py-4 font-bold text-white transition-transform hover:bg-slate-900 active:scale-95">Finalizar Sessão</button>
          </div>
        </div>
      </div>
    );
